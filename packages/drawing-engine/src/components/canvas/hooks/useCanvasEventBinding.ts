...
import type { DrawingTool, HvacElement, Point2D, Room, SectionLineDirection, SymbolInstance2D, Wall } from '../../../types';
...
export function useCanvasEventBinding(
    ...
    useEffect(() => {
        ...
        const handleWallKeyUp = (e: KeyboardEvent) => {
            const { tool, handleWallToolKeyUp, handleRefrigerantPipeKeyUp } = latestOptionsRef.current;
            if (tool === 'wall' || tool === 'partition-wall') {
        ...
        const handleObjectModified = (event: fabric.CanvasEvents['object:modified']) => {
            ...
            if (hvacId) {
                ...
                if (isRefrigerantPipeElementType(existing.type)) {
                    ...
                    const changed =
                        Math.abs(deltaMm.x) > 0.01 ||
                        Math.abs(deltaMm.y) > 0.01;

                    if (changed) {
                        updateHvacElement(hvacId, {
                            position: nextPosition,
                            properties: translateRefrigerantPipeElementProperties(existing.type, existing.properties, deltaMm),
                        });
                    }
